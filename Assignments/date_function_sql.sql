...
select last_day(date('2022-11-17')) as last_day_curr_month;


-- GET LAST DAY OF PREVIOUS MONTH
SELECT LAST_DAY(CURRENT_DATE - INTERVAL '1 MONTH') AS LAST_DAY_PREV_MNTH;
SELECT LAST_DAY(date('2022-11-17') - INTERVAL '1 MONTH') AS LAST_DAY_PREV_MNTH;
SELECT LAST_DAY(date('2022-11-17') - INTERVAL '3 MONTH') AS LAST_DAY_PREV_MNTH;

select day(date('2022-11-17') - interval '20 days') as days_20_back_from_given_date;

select day(date('2022-11-17') - interval '3 weeks') as weeks_3_back_from_given_date_as_day;
select date(date('2022-11-17') - interval '3 weeks') as weeks_3_back_from_given_date_as_date;
...
SELECT QUARTER(date('2022-11-17')) AS QTR_no;
...
SELECT to_date('23-08-2022','dd-mm-yyyy');-- convert date format in standard yyyy-mm-dd


SELECT TO_DATE('1993-08-17') AS DATE;

SELECT TO_CHAR(TO_DATE('1993-08-17'),'DD-MM-YYYY') AS DATE_DD_MM_YYYY; --THIS WILL BE HIGHLY USED/todate will not give this but give standard format
...
select datediff('MONTH', '2021-08-21','2022-06-01');

select datediff('WEEK', '2021-08-21','2022-06-01');

select datediff('QTR', '2014-06-01',CURRENT_DATE);
select datediff('QTR', '2014-06-01','2015-06-01');
